...
1 (DESI/DR1) <https://data.desi.lbl.gov/doc/releases/dr1>`_ and SDSS
DR17, and direct distances from the `Local Volume Database (LVD)
...
The closest quality-filtered SDSS DR17 spectrum matched within 3.5 arcsec
of the galaxy position (see :ref:`redshifts-distances`). Sentinels: ``Z_SDSS``
...
**DESI DR1**, **SDSS DR17**, **NED**, and the **Local Volume Database
(LVD)** — and one additional distance source (NED) for galaxies without
LVD distances.
...
Applied to all spectra within 3.5 arcsec of the SGA position:
...
A single fixed-radius (3.5 arcsec) positional match against SDSS DR17
``specObj``, pre-filtered to ``SPECPRIMARY = 1``, ``CLASS`` in
(``GALAXY``, ``QSO``), and clean redshift warnings (``ZWARNING_NOQSO = 0``
for QSOs, ``ZWARNING = 0`` for galaxies). ``Z > 1.0`` is dropped as
physically implausible for this sample (mirrors the DESI cut above), and
the closest remaining match is kept when more than one falls within the
radius. This match is independent of NED's own SDSS-derived redshifts (see
``NED_FLAG`` below) and takes priority over them, since NED does not
propagate SDSS's own quality flags.

**NED redshift resolution**

NED's aggregated ("fiducial") ``Z_NED`` is replaced by a value drawn from
...
Candidate measurements are grouped into clusters of mutually agreeing
values; if one cluster dominates, its highest-quality measurement is
adopted. Otherwise, or if no candidates exist, the result is flagged rather
than silently trusted (see ``NED_FLAG`` / ``Z_FLAG`` in :ref:`bitmasks`).
...
are all measured and SDSS/NED agree with each other but not with DESI, the
adopted redshift is replaced by SDSS's and flagged ``CONSENSUS_OVERRIDE``
(see :ref:`bitmasks`).

**Automatic NED override**

This sample is overwhelmingly z < 0.3, so an adopted DESI/SDSS redshift
above 0.3 with an independent NED measurement on file is treated as a fiber
landing on a background source: the adopted redshift is replaced by NED's
and flagged ``AUTO_NED_OVERRIDE`` (see :ref:`bitmasks`).

**Manual VI overrides**

A small residual of flagged redshifts (see ``Z_FLAG`` in :ref:`bitmasks`)
are resolved by hand and fed back in as a worksheet CSV keyed by ``SGAID``,
overriding or discarding the adopted redshift and setting ``VI_REVIEWED``.

**Adopted distance (DIST, DIST_IVAR, DIST_REF)**

Priority order **LVD > NED**:

1. ``DIST_REF = LVD`` when ``DIST_LVD > 0``.
2. ``DIST_REF = NED`` when ``DIST_NED > 0``.

LVD distances are converted from kpc to Mpc. ``DIST_IVAR = 0`` may mean no
distance is available, or that no formal uncertainty was reported.

**Peculiar-velocity-corrected redshift (Z_COSMO, Z_COSMO_IVAR)**

For every galaxy with ``Z_IVAR > 0``, the heliocentric ``Z`` is converted to
...
.. list-table::
   ...
   * - ``Z_CENSORED_SDSS``
     ...
     - Adopted default came from an SDSS-compilation reference; discarded
       in favor of the direct SDSS DR17 match (see :ref:`redshifts-distances`)
...
.. list-table::
   ...
   * - ``CONSENSUS_OVERRIDE``
     - 16
     - DESI disagreed with SDSS and NED, which agreed with each other;
       the adopted redshift was replaced by SDSS's
   * - ``AUTO_NED_OVERRIDE``
     - 32
     - Adopted DESI/SDSS redshift exceeded 0.3 and was automatically
       replaced by an independent NED measurement
   * - ``VI_REVIEWED``
     - 64
     - Adopted redshift was set or confirmed by a manual visual-inspection
       pass; the final, human-authoritative bit
