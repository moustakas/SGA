...
**Nominal photometry** (default aperture/method; see ``ELLIPSEPHOT`` for the
full aperture grid and curve-of-growth fits)
...
.. list-table::
   ...
   * - ``DIST_NED_DIRECT``
     ...
     - NED "gold-tier" direct distance, from a single high-precision
       literature measurement (Mpc; 0 = none available)
...
optical/infrared colors, an apparent magnitude, surface brightness,
isophotal diameter, axis ratio, and Galactic extinction as features.
Apparent magnitude and surface brightness use the *r* band, falling back to
*i* and then *g* when *r* is unavailable. We validate the model with 5-fold
cross-validation, then retrain it on the full spectroscopic subsample and
predict ``Z_PHOT`` for every galaxy in the catalog. ``Z_PHOT_IVAR`` is the
inverse variance of the spread of predictions across the individual trees
of the forest.
...
.. note::
   ...
   Gold-tier method abbreviations, in priority order, with their typical
   distance-modulus uncertainty (mag) inferred from the literature values
   compiled in NED:
...
.. list-table::
   ...
   * - ``DISCREPANCY``
     ...
     - Any two measured redshifts (LVD, DESI, SDSS, NED) differ by more
       ...
       literature NED redshift can exceed this against a precise DESI/SDSS
