...
measured redshift — a Hubble-flow distance derived from ``Z_COSMO``.

**Adopted distance (DIST, DIST_IVAR, DIST_REF, DIST_METHOD)**

Priority order **NED direct > LVD > NED mean > Hubble flow**:

1. ``DIST_REF = NED_DIRECT`` when a high-precision "gold-tier" NED distance
   is available (Cepheids, TRGB, PNLF, SBF, SNIa, SNII, GCLF, or SNIa-SDSS).
   If the best available gold-tier method rests on a single, unreplicated
   reference and conflicts with a more consistent measurement from another
   gold-tier method, we adopt the latter instead.
...
4. ``DIST_REF = ZCOSMO`` for every remaining galaxy with a measured
   redshift: a Hubble-flow distance, ``DIST = c`` × ``Z_COSMO`` / 70 km/s/Mpc.
