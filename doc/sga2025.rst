...
for QSOs, ``ZWARNING = 0`` for galaxies). The closest match is kept when
more than one falls within the radius. This match is independent of NED's
own SDSS-derived redshifts (see ``NED_FLAG`` below) and takes priority over
them, since NED does not propagate SDSS's own quality flags.
