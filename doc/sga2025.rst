...
3. Fall back to any ``ZWARN = 0`` spectrum regardless of ``SPECTYPE``.
4. Among survivors, select the spectrum with the highest ``DELTACHI2``.

If no ``ZWARN = 0`` spectrum survives, no DESI redshift is adopted for that
