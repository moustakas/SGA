...
redshifts (see ``NED_FLAG`` below) and takes priority over them, since NED
...
than silently trust it (see ``NED_FLAG`` / ``Z_FLAG`` in :ref:`bitmasks`).
...
1. ``DIST_REF = NED_DIRECT`` when a high-precision "gold-tier" NED distance
   ...
   -- as opposed to Tully-Fisher, Fundamental Plane, and other
   lower-precision scaling relations). If the best available gold-tier
   method rests on a single, unreplicated reference and conflicts with a
   more consistent measurement from another gold-tier method, we adopt the
   latter instead.
...
For any adopted distance with no reported uncertainty (common for LVD), we
assign a 15% fractional floor rather than ``DIST_IVAR = 0``.
...
We may set multiple bits simultaneously.
