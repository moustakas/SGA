...
This logic handles two distinct problems, both resolved against the full set
of NED redshift measurements for the object (not just its default/fiducial
value):

- NED occasionally assigns a bogus, very high "fiducial" redshift to an
  object -- most often SDSS fiber contamination from a background source
  overlapping a nearby dwarf galaxy -- while its own full list of redshift
  measurements for that object contains the correct, much lower value.
  Triggered when the adopted ``Z_NED`` exceeds a threshold (0.4 by default;
  true SGA-2025 redshifts are almost never this high).
- NED's own aggregated default has no redshift at all for the object, even
  though its full measurement list does (e.g. UGC 06451, whose only
  overview-level redshift is a stale/bogus WISE photometric estimate but
  which has real spectroscopic measurements in NED's full list). Triggered
  whenever the default ``Z_NED`` is missing, regardless of threshold.

In both cases the full set of NED redshift measurements for that object is
queried and used to look for a good answer:
...
3. Otherwise -- multiple candidates that disagree with each other, or none
   ...
   kept unchanged if there are no candidates at all (for a missing default
   with no candidates, ``Z_NED`` is simply left unset -- the ordinary case of
   a genuinely unmeasured object, not worth flagging). Otherwise the result
...
Bits ``0x02``, ``0x04``, and ``0x08`` address a blind spot in the discrepancy
check: when NED is the *only* redshift measurement available (no LVD or DESI
value at all), there is nothing to cross-check it against, so a still-
suspect, unresolved, or missing-default NED redshift (see ``NED_FLAG`` above)
would otherwise pass through silently. In that single-measurement case only,
the corresponding ``NED_FLAG`` bits are mirrored into ``Z_FLAG``:
...
- ``0x04`` -- the sole measurement's ``NED_FLAG`` has ``Z_UNRESOLVED`` set:
  ...
  implausibly high).
- ``0x08`` -- the sole measurement's ``NED_FLAG`` has ``Z_MISSING`` set:
  NED had no default redshift at all, and the adopted ``Z`` was pulled from
  NED's full list of redshift measurements instead (e.g. UGC 06451).

None of these bits trigger any automatic correction; all three are intended
purely as a signal for targeted visual inspection.
...
.. list-table::
   ...
   * - ``NED_MISSING``
     - 8
     - NED is the sole redshift measurement (no LVD or DESI value at all)
       and its ``NED_FLAG`` has ``Z_MISSING`` set: NED had no default
