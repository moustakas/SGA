...
SGA  (/global/common/software/desi/users/ioannis/SGA)
  ...
  legacypipe, mpi4py (built from source against Cray MPICH), pydl,
...
3. Build mpi4py from source against Cray MPICH (`MPICC="$(which cc) -shared" pip install --no-binary=mpi4py mpi4py`) — **do not** conda/pip install mpi4py
4. `pip install` — pydl, tractor, legacypipe, SGA, isoster, imagine
5. Deploy `activate-sga.sh` to `SGA_PREFIX/etc/activate.sh`
...
bash etc/update-env-sga.sh mpi4py       # rebuild mpi4py (e.g. after a Cray MPICH update)
...
**Note on mpi4py:** the build captures the Cray `cc` path from the current shell before activating
the conda env (where the conda-packaged gcc would otherwise shadow it). If the build fails with
`mpi.h: No such file or directory`, ensure `PrgEnv-gnu` is loaded: `module load PrgEnv-gnu`.
...
registering the Jupyter kernel. `mpi4py` is included directly in
`environment-laptop-sga.yml` (conda-forge bundles OpenMPI on macOS; no
source build needed):
...
### Optional: pvhub (peculiar-velocity corrections)

[`pvhub`](https://github.com/KSaid-1/pvhub) predicts the peculiar velocity at
any (RA, Dec, z) from published linear velocity-field reconstructions
(2M++, 2MRS, SDSS-based), used by `bin/SGA2025-build-catalog` to compute
flow-corrected redshifts/distances for objects with no NED match. It is not
a proper pip package (no `setup.py`/`pyproject.toml`) and ships its velocity
maps as large files via Git LFS, so it is not part of
`create-env-laptop-sga.sh` — install it once, separately:

```bash
brew install git-lfs
git lfs install
git lfs clone https://github.com/KSaid-1/pvhub.git ~/code/pvhub
```

Make it importable in the `SGA` env with a `.pth` file (same approach used
for `imagine` on NERSC):

```bash
micromamba activate SGA
SITE=$(python -c "import site; print(site.getsitepackages()[0])")
echo ~/code/pvhub > "$SITE/pvhub.pth"
python -c "import pvhub"   # sanity check
```
...
| `environment-laptop-sga.yml` | Conda env spec for SGA (laptop; includes mpi4py via conda-forge) |
...
| `update-env-sga.sh` | Update or editable-install individual SGA packages (NERSC); includes `mpi4py` rebuild |
| `update-env-sgaml.sh` | Update individual SGAML packages (NERSC) |
| `sga-env.sh` | SLURM job environment template; copy to working dir and edit before running `generate-sga-slurm` |
| `README.sga-slurm` | Step-by-step SLURM workflow for `generate-sga-slurm` |
...
| `~/code/pvhub` | External clone (Git LFS) for optional peculiar-velocity corrections; see Laptop — SGA |
