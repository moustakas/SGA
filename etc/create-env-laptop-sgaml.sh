#!/bin/bash
# Build the SGAML (SGA + machine learning) environment on a laptop (macOS).
# Targets Apple Silicon with the MPS backend for PyTorch (no CUDA).
# Python 3.12 matches the NERSC pytorch/2.11.0 module.
...
# ---------------------------------------------------------------------------
# Step 1: conda packages (compiler, swig, C libraries, pytorch, numpy, etc.)
# ---------------------------------------------------------------------------
echo "==> Creating conda environment '$ENV_NAME'..."
micromamba create -n $ENV_NAME --file etc/environment-laptop-sgaml.yml --yes
...
# ---------------------------------------------------------------------------
# Step 3: pip installs — run after the env is created so the C compiler and
...
# Use Python for the legacypipe version patch (cross-platform; avoids sed -i
# syntax differences between macOS and Linux).
# ---------------------------------------------------------------------------
echo ""
...
# zoobot[pytorch] pulls in webdataset, litmodels, galaxy-datasets, and
# remaining deps. torch/torchvision/lightning/torchmetrics are already
# conda-installed above and will not be reinstalled.
echo ""
...
$RUN python -m ipykernel install --user --name $ENV_NAME --display-name "SGAML"
