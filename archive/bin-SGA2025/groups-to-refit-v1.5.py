#!/usr/bin/env python
"""
groups-to-refit-v1.5.py

Combined script that:
  1. Restores v1.5 groups to v1.4 group definitions where valid
  2. Identifies all groups needing refit (new, mult changed, moved, overlay,
     truncated, or predetermined)
  3. Writes rem-ellipse-v1.4.txt  (ellipse-only removal, wildcard pattern)
     and rem-coadds-v1.4.txt      (full directory removal)
  4. Writes viewer-truncated.fits, viewer-large-refit-v1.5.fits,
     and viewer-ellipse-refit-v1.5.fits

Restoration criteria (groups skipped if any fail):
  - No objects dropped from v1.4 group (drops.csv)
  - No objects newly added in v1.5 (adds.csv)
  - No member ellipse truncated by v1.4 mosaic boundary

Refit sources (all full footprint):
  1. New GROUP_NAMEs in v1.5 not in v1.4
  2. GROUP_MULT changed for shared group names
  3. Object moved groups between v1.4 and v1.5
  4. Object in updates.csv or flags.csv changes between v1.4 and v1.5
  5. Truncation: member ellipse exceeds v1.4 mosaic boundary
  6. Predetermined refit list (SGA2025-v1.4-refit.fits, optional)
"""
...
def report_large_refit_groups(p_new, p_base, all_refit, source_sets, mosaic_diam_map=None, max_rows=50):
    p_new_names  = np.asarray(p_new['GROUP_NAME']).astype(str)
    p_base_names = np.asarray(p_base['GROUP_NAME']).astype(str)
    ...
    def print_table(dedup, label, total):
        ...
        for reg in regions:
            ...
            if not reg_dedup:
                continue
            shown = reg_dedup[:max_rows]
            truncated = len(reg_dedup) - len(shown)
            suffix = f", {truncated} more not shown" if truncated else ""
            log.info(f"  [{reg}] ({len(reg_dedup)} objects{suffix})")
            log.info(hdr)
            log.info(sep)
            for objname, (g, d, info) in shown:
    ...
    print_table(dedup_coadds,
                "COADDS REFIT — groups > 5 arcmin (rem-coadds-v1.4.txt)",
                len([g for g in all_refit if not is_ellipse_only(g)]))
    print_table(dedup_ellipse,
                "ELLIPSE-ONLY REFIT — all groups (rem-ellipse-v1.4.txt)",
...
def main(base_version='v1.4', new_version='v1.5',
         min_diam=1e10, trunc_margin=10.0, verbose=False, debug=False,
         max_rows=50):
    # min_diam defaults to 1e10 so that only OPT_IN_GROUPS are restored
    # by default. Pass 0.0 to attempt restoration of all groups.
    ...
    # Derive opt-in groups: all large groups in p_base (GROUP_DIAMETER > 7'),
    # regardless of whether they appear in p_new. The opt-in mechanism bypasses
    # all restoration criteria, so these groups are always restored to their
    # base-version properties — protecting expensive mosaics from accidental
    # deletion even when membership changed between versions.
    p_base_names = np.asarray(p_base['GROUP_NAME']).astype(str)
    opt_in_mask  = (np.asarray(p_base['GROUP_PRIMARY'], dtype=bool) &
                    (np.asarray(p_base['GROUP_DIAMETER'], dtype=float) > 10.))
    opt_in_groups = list(p_base_names[opt_in_mask])
    log.info(f"  Opt-in groups (GROUP_DIAMETER>10'): {len(opt_in_groups):,d}")
    ...
    # Validate that no opt-in group ended up in rem-coadds
    opt_in_set = set(opt_in_groups)
    coadds_violations = [gdir for gdir in coadds_dirs
                         if gdir.rstrip('/').split('/')[-1] in opt_in_set]
    if coadds_violations:
        log.warning(f"WARNING: {len(coadds_violations)} opt-in group(s) found in "
                    f"{rem_coadds} — these mosaics would be deleted!")
        for gdir in coadds_violations:
            log.warning(f"  {gdir}")
        raise ValueError(f"Opt-in groups in rem-coadds — aborting")
    else:
        log.info(f"Validation passed: no opt-in groups in {rem_coadds}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base-version', default='v1.4',
                        help='Base (old) parent version (default: v1.4)')
    parser.add_argument('--new-version', default='v1.5',
                        help='New parent version (default: v1.5)')
    parser.add_argument('--min-diam', type=float, default=1e10,
                        help='GROUP_DIAMETER threshold for restoration '
                             '(arcmin, default 1e10 = opt-in only)')
    ...
    parser.add_argument('--max-rows', type=int, default=50,
                        help='Max rows per region in diagnostic table (default 50)')
