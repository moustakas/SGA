...
def galex_coadds(onegal, galaxy=None, radius=30, pixscale=2.75, 
                 survey=None, galex_dir=None, verbose=False):
    ...
    if galex_dir:
        ...
        for niceband, band in zip(nicegbands, gbands):
            ...
            fitsfile = os.path.join(survey.output_dir, '{}-{}-image.fits'.format(galaxy, niceband))
            if verbose:
                print('Writing {}'.format(fitsfile))
            fitsio.write(fitsfile, coimg, clobber=True)

            fitsfile = os.path.join(survey.output_dir, '{}-{}-model.fits'.format(galaxy, niceband))
            if verbose:
                print('Writing {}'.format(fitsfile))
            fitsio.write(fitsfile, comod, clobber=True)

            fitsfile = os.path.join(survey.output_dir, '{}-{}-resid.fits'.format(galaxy, niceband))
            if verbose:
                print('Writing {}'.format(fitsfile))
            ...
            if False:
                ...
                fig.savefig('{}-{}-image.jpg'.format(opt.prefix, niceband), bbox_inches='tight', pad_inches=0)

                print('Coadded model')
                fig, ax = plt.subplots(figsize=(8, 8))
                dimshow(comod, ticks=False)
                #plt.imshow(comod, interpolation='nearest', origin='lower')
                fig.savefig('{}-{}-model.jpg'.format(opt.prefix, niceband), bbox_inches='tight', pad_inches=0)

                print('Coadded residual')
                fig, ax = plt.subplots(figsize=(8, 8))
                dimshow(coresid, ticks=False)
                #plt.imshow(comod, interpolation='nearest', origin='lower')
                fig.savefig('{}-{}-resid.jpg'.format(opt.prefix, niceband), bbox_inches='tight', pad_inches=0)

        jpgfile = os.path.join(survey.output_dir, '{}-galex-image.jpg'.format(galaxy))
        if verbose:
            print('Writing {}'.format(jpgfile))
        rgb = _galex_rgb(coimgs)
        imsave_jpeg(jpgfile, rgb, origin='lower')
        
        jpgfile = os.path.join(survey.output_dir, '{}-galex-model.jpg'.format(galaxy))
        if verbose:
            print('Writing {}'.format(jpgfile))
        rgb = _galex_rgb(comods)
        imsave_jpeg(jpgfile, rgb, origin='lower')
        
        jpgfile = os.path.join(survey.output_dir, '{}-galex-resid.jpg'.format(galaxy))
        if verbose:
            print('Writing {}'.format(jpgfile))
