...
Functions to handle external catalogs.
...
def read_lvd(rank=0, rows=None, overwrite=False, verbose=True):
    ...
    if not os.path.isfile(lvdfile) or overwrite:
        ...
        # drop Sagittarius and unconfirmed systems
        log.info(f'Dropping Sagittarius and {np.sum(lvd["confirmed_real"]==0):,d}/{len(lvd):,d} unconfirmed dwarfs.')

        I = (lvd['name'] != 'Sagittarius') * (lvd['confirmed_real'] == 1)
        lvd = lvd[I]
...
def read_dr910(rows=None):
    ...
    dr910file = os.path.join(sga_dir(), 'parent', 'external', f'dr910-supplement-{version}.fits')
    dr910 = Table(fitsio.read(dr910file, rows=rows))
    dr910['ROW'] = np.arange(len(dr910))
    log.info(f'Read {len(dr910):,d} objects from {dr910file}')
    return dr910
...
def read_sga2020(rank=0, rows=None, columns=None):
    """Read the SGA-2020 catalog.

    """
    sga2020file = os.path.join(sga_dir(), 'parent', 'external', 'SGA-2020.fits')
    sga2020 = Table(fitsio.read(sga2020file, ext='ELLIPSE', rows=rows, columns=None))
...
def read_wisesize_sample(cat, fullcat=None, catfile=None, region='dr9-north',
    ...
    outfile = os.path.join(cutoutdir, f'zooniverse-{project}-{region}.fits')
